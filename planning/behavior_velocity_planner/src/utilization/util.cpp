  ...
  const double min_distance)
{
  ...
  for (int i = 0; i <= 1; i++) {
    if(static_cast<int>(insert_index + i) == static_cast<int>(path.points.size())){continue;}
    if (tier4_autoware_utils::calcDistance2d(path.points.at(insert_index + i), path_point) < min_distance) {
