  ...
  const std::shared_ptr<const PlannerData> & planner_data)
{
  ...
  // debug code
  {
    ...
    if (debug) {
      for (const auto & p : out_path.points) {
        if (i % interval == 0)
          std::cout << "index: " << i << " out: " << p.point.longitudinal_velocity_mps << std::endl;
        i++;
      }
      i = 0;
      for (const auto & p : in_path.points) {
        if (i % interval == 0)
          std::cout << "index: " << i << " in: " << p.point.longitudinal_velocity_mps << std::endl;
        i++;
      }
    }
  }
