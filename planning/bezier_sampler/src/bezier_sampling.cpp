...
{
std::vector<Bezier> sample(const Configuration & initial, const Configuration & final, const SamplingParameters & params){
    // Sampling method from Section IV of A. Artuñedoet al.: Real-Time Motion Planning Approach for Automated Driving in Urban Environments
    std::vector<Bezier> curves;
    curves.reserve(params.nb_t * params.nb_t * params.nb_k);

    double distance_initial_to_final = std::sqrt((initial.x - final.x) * (initial.x - final.x) + (initial.y - final.y) * (initial.y - final.y));
    // Tangent vectors
    ...
    for (double m_initial = params.mt_min; m_initial <= params.mt_max; m_initial += step_t) {
      ...
      for (double m_final = params.mt_min; m_final <= params.mt_max; m_final += step_t) {
        ...
        for (double k = params.mk_min; k <= params.mk_max; k += step_k) {
          ...
          Eigen::Vector2d initial_acceleration = acceleration_length * initial_tangent_unit +
                                                 initial.curvature * initial_tangent_length *
                                                 initial_tangent_length * initial_normal_unit;
          Eigen::Vector2d final_acceleration = acceleration_length * final_tangent_unit +
                                               final.curvature * final_tangent_length *
                                               final_tangent_length * final_normal_unit;
          curves.push_back(generate(
            {initial.x, initial.y}, {final.x, final.y}, initial_tangent_unit*initial_tangent_length, initial_acceleration,
...
Bezier generate(
  const Eigen::Vector2d & initial_pose, const Eigen::Vector2d & final_pose, const Eigen::Vector2d & initial_velocity,
  const Eigen::Vector2d & initial_acceleration, const Eigen::Vector2d & final_velocity,
  const Eigen::Vector2d & final_acceleration)
{
  ...
  control_points.row(2) =
    2 * control_points.row(1) - control_points.row(0) + (1.0 / 20.0) * initial_acceleration.transpose();
  control_points.row(3) =
    2 * control_points.row(4) - control_points.row(5) - (1.0 / 20.0) * final_acceleration.transpose();
